...
export async function register(
  ...
  await pool.query(`
  CREATE TABLE IF NOT EXISTS user40 (
  ...
  const existingRecordResult = await pool.query(
    "SELECT * FROM user40 WHERE phoneNumber = $1",
    [phoneNumber]
  );
  if (existingRecordResult.rows.length > 0) {
    ...
    if (sms) {
      const updateExistingRecordResult = await pool.query(
        "UPDATE user40 SET otp = $1, currentTime = $2 WHERE phoneNumber = $3 RETURNING *;",
    ...
    if (sms) {
      const insertResult = await pool.query(
        "INSERT INTO user40 (phoneNumber, otp, username, role, currentTime) VALUES ($1, $2, $3, $4, $5) RETURNING *;",
...
export async function login(id: any, otp: any): Promise<any> {
  console.log("first.....",id,otp)
  const user = await pool.query(
    "SELECT * FROM user40 WHERE id = $1 AND otp = $2",
    [id,otp]
  );
console.log("heyy",user.rows)
  if (user.rows.length === 0) {
    throw new HttpException(400, "No user found with the provided ID and OTP");
  }

  await pool.query("UPDATE user40 SET otp = null WHERE id = $1", [id]);
  const verificationTime = await pool.query(
    "SELECT currentTime FROM user40 WHERE id = $1",
    [id]
  );
  console.log(
    "verify",
  ...
  if (withinTimeLimit) {
    return { token: token, role: user.rows[0].role, id: id,name:user.rows[0].username };
  } else {
